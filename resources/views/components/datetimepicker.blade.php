<div class="">
    <div x-data="app" x-cloak>
        <div class="container mx-auto py-2">
            <div>
                <input type="text" name="date_from" class="hidden" x-model="dateFromYmdHis">
                <input type="text" name="date_to" class="hidden" x-model="dateToYmdHis" class="mt-2 sm:mt-0 ml-0 sm:ml-2">
                <div class="relative" @keydown.escape="closeDatepicker()" @click.outside="closeDatepicker()">
                    <div class="inline-flex items-center border rounded-md mt-3 bg-gray-200">
                        <input type="text" @click="endToShow = 'from'; init(); showDatepicker = true" x-model="outputDateFromValue" :class="{'font-semibold': endToShow == 'from' }" class="date-from-input focus:outline-none border-none p-2 w-64 rounded-l-md border-r border-gray-300"/>
                        <div class="inline-block px-2 h-full">До</div>
                        <input type="text" @click="endToShow = 'to'; init(); showDatepicker = true" x-model="outputDateToValue" :class="{'font-semibold': endToShow == 'to' }" class="date-to-input focus:outline-none border-none p-2 w-64 rounded-r-md border-l border-gray-300"/>
                    </div>
                    <div
                        class="absolute bg-white z-10 mt-2 rounded-lg shadow p-4 sm:w-80 w-full"
                    ...
                    >
                        <div class="flex flex-col items-center">
                            ...
                            <div>
                                <button @click="showDatepicker = false" class="px-2 py-1 border border-gray-300 hover:border-gray-500 rounded-md">Отмена</button>
                                <button @click="outputDateValues(); showDatepicker = false" class="px-2 py-1 border border-blue-600 bg-blue-500 hover:bg-blue-300 text-white rounded-md">Ок</button>
                            </div>
...
<script>
    const MONTH_NAMES = ['Январь', 'Ферваль', 'Март', 'Апрель', 'Май', 'Июнь', 'Июль', 'Август', 'Сентябрь', 'Октябрь', 'Ноябрь', 'Декабрь'];
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('app', () => ({
            ...
            outputDateValues() {
                ...
                this.endToShow = '';
                if (document.statisticData){
                    document.statisticData.changeDatetime(this.dateFromYmdHis,this.dateToYmdHis);
                }
                if (document.dataTable){
                    document.dataTable.changeDatetime(this.dateFromYmdHis,this.dateToYmdHis);
                }
            },
